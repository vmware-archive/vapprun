...
class Property:
    ...
    def __init__(self, key="", type="", value="", userConfig=True):
        ...
        if self.isMacro(): self.userConfig = False    
    ...
    def effectiveValue(self, deployParams, parentEnv=set()):
        ...
        if cmd == None:
            print "Error: Invalid expression '%s' for property '%s'" % (self.value, self.key)
            sys.exit(1)
            
        if arg == None:
            # Assignment
            if not cmd in parentEnv:
                print "Error: Undefined reference '%s' for property '%s'" % (cmd, self.key)
                sys.exit(1)
            return parentEnv[cmd]
        else:
            if arg != "Network":
                print "Error: Invalid network name '%s' for property '%s' (it must be 'Network')" % (arg, self.key)
                sys.exit(1)
            (val, found) = vapps.ipPool.lookupChildTextNode(cmd)            
            if not found:
                print "Error: Invalid expression  '%s' for property '%s'" % (self.value, self.key)
                sys.exit(1)
    ...
    def getAssignee(self):
        (cmd, arg) = self.parseMacro()
        if cmd == None or arg != None: return None
        return cmd
...
class Link:
    ...
    def asString(self):
        return "[startOrder: %d  startWait: %d waitForTools: %s stopWait: %d]" % \
               (self.startOrder, self.startWait, str(self.waitForTools), self.stopWait)
...
class DeployParams:
    ...
    def load(self, fileName):
        ...
        self.allocationPolicy = node.getAttr("allocationPolicy", self.allocationPolicy)
        
        for n in node.children:
            if n.value != None and n.tag in self.config:
    ...
    def isAutoIpKey(self, key):
        return not self.isFixedIpPolicy() and key in ipKeys
...
class Entity:
    ...
    def load(self):
        ...
        if node.tag != self.getRootTag():
            print "Error reading", self.cfgPath, "Invalid root element:", node.tag
            sys.exit(1)                                          
    ...
    def getUsedFiles(self, usedList):
        if self.parent == None:
            usedList.append((os.path.realpath(os.path.join(self.dir, "deploy.cfg")), False))
...
class VmEntity(Entity):
    ...
    def startChild(self, indent = 0):
        ...
        while(waited < self.link.startWait):
            time.sleep(1)
            if waited%10 == 0:
                print spc+"Waiting for %d secs..." % (self.link.startWait - waited)
            waited += 1
...
class VAppEntity(Entity):
    ...
    def getRootTag(self): return "vapp"
...
class VAppInventory:
    ...
    def __init__(self, path):
        ...
        self.cfgFile = os.path.realpath(os.path.join(self.dir, WORKSPACE_CFG_NAME))
        try:
            self.config = ReadXmlDoc(self.cfgFile)            
        except:
            print "Error: Cannot parse", WORKSPACE_CFG_NAME, "Reason:",sys.exc_info()[1]
            sys.exit(1)               
